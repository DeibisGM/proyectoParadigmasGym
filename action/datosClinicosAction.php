...
try {
    // Verificar si es usuario logueado (cliente) o admin
    ...
    if(isset($_POST['create'])){
        ...
        if ($esUsuarioCliente) {
            // Para clientes: solo pueden crear sus propios datos
            ...
            // Forzar que use su propio ID
            $clienteId = $_SESSION['usuario_id'];

        } else if ($esAdmin) {
            // Para admin: validación normal
            if(empty($clienteId)){
                $response['success'] = false;
        ...
        // Validaciones según tipo de usuario
        if ($esUsuarioCliente) {
            // Para clientes: solo pueden actualizar sus propios datos
            if (!isset($_SESSION['usuario_id'])) {
                $response['success'] = false;
                $response['message'] = 'Error: Usuario no autenticado.';
                echo json_encode($response);
            ...
            // Verificar que el registro le pertenece
